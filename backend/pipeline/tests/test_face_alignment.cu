...
int main() {
  ...
  std::vector<std::uint8_t> input(kPixels * 3);
  for (int y = 0; y < mvision::kAlignedFaceHeight; ++y) {
    for (int x = 0; x < mvision::kAlignedFaceWidth; ++x) {
      ...
      input[pixel * 3] = static_cast<std::uint8_t>(x);
      input[pixel * 3 + 1] = static_cast<std::uint8_t>(y);
      input[pixel * 3 + 2] = 77;
    }
  }
  ...
  const std::size_t pitch = mvision::kAlignedFaceWidth * 3;
